...
### 迷失女仆的生成与行为
- **生成机制**：在主世界，并且满足相关条件下，自然刷新“迷失女仆”（暂定名称）。这些女仆作为敌对生物，会自动寻找并攻击附近的玩家。
- **装备系统**：迷失女仆会随机装备游戏中现有的武器和护甲。
- **战斗模式**：目前支持近战，弓箭远程，卓越前线枪械。(画饼：等拔刀剑更新到1.21.1后会及时同步相关内容)
...
请查看Mod配置文件，一般情况下，它的路径是 ./config/touhou_lost_maid-server.toml。
...
2. Mekanism
    ...
    已知在同时安装此Mod与Mekanism的情况下，“偶然”会发生崩溃，原因尚不明确。

    作者在他的电脑上Everything is Ok.
    
    因此，我们建议您在慎重考虑后再尝试同时安装两者（当然这通常不会导致存档损坏）。

    目前无法稳定复现该问题，如果您遇到了相关问题，请通过加群反馈或者提交issue的方式报告，感谢您为此做出的贡献。